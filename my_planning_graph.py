...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            ...
            for s_node in s_levels:

                # Connect action to current level if a_node prenodes are defined
                # in S level
                if a_node.prenodes.issubset(s_levels):
                    self.a_levels[level].add(a_node)

                if s_node in a_node.prenodes:
                    # Connect to the appropriate level
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        for n1_p, n2_p in product(node_a1.parents, node_a2.parents):
            if n1_p.is_mutex(n2_p):
                return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        for n1_p, n2_p in product(node_s1.parents, node_s2.parents):
            if not n1_p.is_mutex(n2_p):
                return False
        return True
